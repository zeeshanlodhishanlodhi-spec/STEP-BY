...
function StepbyApp() {
  ...
  const handleImageFiles = async (fileList) => {
    ...
    try {
      ...
      // New photos go to the FRONT of the list, so they become the main
      // photo shown on the store/admin card straight away — previously
      // they were added to the end, so on an edit the OLD photo kept
      // showing as "the" photo even after a new one was added, which
      // looked exactly like the new photo never saved at all.
      setProductForm((f) => {
        const images = [...uploaded, ...(f.images || [])];
  ...
  const saveProduct = async (e) => {
    e.preventDefault();
    if (imgUploading) {
      showToast("Photo abhi upload ho rahi hai — thodi der ruk kar dobara try karein.");
      return;
    }
    const f = productForm;
            ...
            <div className="sb-modal-box">
              ...
              <form onSubmit={saveProduct}>
                ...
                <div className="sb-form-row">
                  ...
                  {(productForm.images || []).length > 0 && <div style={{ fontSize: 10, color: "#888", marginTop: 4 }}>First photo (gold border) is used as the main image — newly added photos go to the front automatically.</div>}
                </div>
                ...
                <button className="sb-btn sb-btn-dark" style={{ width: "100%", marginTop: 6 }} type="submit" disabled={imgUploading}>
                  {imgUploading ? "UPLOADING PHOTO…" : "SAVE PRODUCT"}
                </button>
              </form>
            </div>
          </div>
