...
import { db, auth, storage, ADMIN_EMAIL } from "./firebase.js";
...
function StepbyApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDoc(fsDoc("orders"));
        if (!cancelled && snap.exists() && Array.isArray(snap.data().value)) {
          setOrders(snap.data().value);
        }
      } catch (e) { console.error("Firestore load failed for orders", e); }
    })();
    ...
    const unsub = onSnapshot(
      ...
      async (snap) => {
        ...
        if (Array.isArray(value) && value.length) {
          ...
          // First-ever run for this Firebase project: nothing saved yet, so seed it.
          seeded = true;
          try { await setDoc(fsDoc("products"), { value: seedProducts }); } catch (e) { console.error("Firestore seed failed for products", e); }
        }
        setLoaded(true);
      },
  ...
  const persist = async (key, value, shared) => {
    // key arrives as "stepby:products" etc. — strip the "stepby:" prefix for the Firestore doc id.
    const shortKey = key.replace(/^stepby:/, "");
    if (!shared) {
      try { window.localStorage.setItem(key, JSON.stringify(value)); } catch (e) { console.error("Local save failed", key, e); }
      return;
    }
    try { await setDoc(fsDoc(shortKey), { value }); } catch (e) { console.error("Firestore save failed", key, e); }
  };
  ...
  const addToCart = (product) => {
    ...
    setCart(next);
    persist("stepby:cart", next, false);
    ...
    return (
      <div className="sb">
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 14 }}>
          <img src={LOGO} alt="STEPBY" style={{ height: 70 }} />
          <p style={{ fontSize: 12, letterSpacing: 1, color: "#888" }}>LOADING STEPBY…</p>
        </div>
      </div>
    );
  }
  ...
  const compressImage = (file, maxDim = 1000, quality = 0.75) =>
    new Promise((resolve) => {
  ...
  const saveProduct = (e) => {
    ...
    const exists = products.some((p) => p.id === built.id);
    const nextProducts = exists ? products.map((p) => (p.id === built.id ? built : p)) : [built, ...products];
    setProducts(nextProducts);
    persist("stepby:products", nextProducts, true);
    setProductForm(null);
  ...
  const deleteProduct = (id) => {
    ...
    setCart(nextCart);
    persist("stepby:products", nextProducts, true);
    persist("stepby:cart", nextCart, false);
  };
  ...
  const placeOrder = (e) => {
    ...
    persist("stepby:orders", nextOrders, true);

    // Reduce stock for every ordered item; anything that hits 0 automatically
    // flips to Out of Stock and sinks to the bottom of the store (see filteredProducts).
    const orderedQty = {};
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 8 }}>
                      {productForm.images.map((url, idx) => (
                        <div key={idx} style={{ position: "relative" }}>
                          <img src={url} alt={`Photo ${idx + 1}`} style={{ height: 70, width: 70, borderRadius: 6, objectFit: "cover", border: idx === 0 ? "2px solid var(--gold)" : "1px solid var(--line)" }} />
          ...
          <div className="sb-cart-items">
            ...
            {cartLines.map((l, i) => (
              <div className="sb-cart-row" key={i}>
                <img src={l.product.img} alt={l.product.name} loading="lazy" decoding="async" />
                <div>
